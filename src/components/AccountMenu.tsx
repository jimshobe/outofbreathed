...
export default function AccountMenu() {
  ...
  useEffect(() => {
    ...
    if (user.uid === ADMIN_UID) { setStatus('approved'); return; }
    getDoc(doc(db, 'users', user.uid)).then((snap) => {
      if (snap.exists()) setStatus(snap.data().status as UserStatus);
  ...
  const statusLabel: Record<NonNullable<UserStatus>, string> = {
    approved: 'Approved to comment',
    pending: 'Pending approval to comment',
    banned: 'Commenting not permitted',
  };
            ...
            <div className="account-user">
              <div className="account-user-info">
                ...
                <div>
                  ...
                  {status && <div className="account-user-status">{statusLabel[status]}</div>}
                </div>
              </div>
